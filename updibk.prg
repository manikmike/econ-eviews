' This routine creates disability ratios and stores them in an operational workfile (op123xo.wf1), using raw data stored in a raw data workfile (op123xr.wf1). 
...
' At the end, all series are uploaded to the operational workfile, op123xo.wf1, for use in TR24. 

' Note: As currently written, this program must be run 3 separate times in order to obtain series for all Alts (1, 2, and 3). 
' Each time, %input1 and %input2 should be changed accordingly, to reference the correct op workfiles for the desired Alt.
...
wfopen C:\Users\715256\GitRepos\econ-eviews\{%input1}.wf1
wfselect work
pageselect a

copy {%input1}::a\pm20dicp work::a\temp
...
for !a = 15 to 66
   for %s m f
      genr n{%s}{!a}d = {%input1}::p{%s}{!a}dicp / 1000000
   next
next
...
for !a = 15 to 69
   for %s m f
      genr n{%s}{!a}di = {%input1}::p{%s}{!a}dins / 1000000
   next
next
...
wfopen C:\Users\715256\GitRepos\econ-eviews\{%input2}.wf1
wfselect work
...
'close @wf
'logmsg store the created annual and quarterly data in op121xo bank, and close the bank
...
copy n*d {%input2}::a\n*d 
copy n*di {%input2}::a\n*di  
copy r*di {%input2}::a\r*di 
copy r*d {%input2}::a\r*d 
wfselect {%input2}
wfsave(2) {%input2}

wfselect work
pageselect q
copy r*di {%input2}::q\r*di
copy r*d {%input2}::q\r*d
