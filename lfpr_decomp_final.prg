...
'User shoudl set the parameters listed under "****SET these parameters before running the program *****.
...
!TRyr = 2017 'the LFPR decomposition table will start from TRyr-1 Q4, i.e. Q4 of the last historical year. NOTE: this program assumes that TRyr is in the form 20XX. The program will NOT work for TRyr 1999 or earlier (nor will it work for TRyr 2100 and later, but this won't be a problem for a while).

!endyr=2099 'end year for the LFPR decomposition table, usually the last year of the TR projection period, i.e. the latest year in the TR databanks

'NOTE: this program assumes that databanks for the final TR run are located in \\s1f906b\econ\Aremos\TR20XX Banks. If this changes, adjust the variable %folder below
%folder="\\s1f906b\econ\Aremos\TR"+@str(!TRyr)+"Banks\" 'folder that contains final databanks for the TR run

%LFPRestimates="E:\usr\pvlasenk\EViews\LFPR decomp\LFPR_coefs.wf1" 'FULL path to the workfile that contains LFPR coefficient matrix (including the filename)

'now set the parameters that determine which output is to be produced

'group(s) for which the decomposition is to be done, space delimited
%user_groups="f16o m16o 16o"
...
'%lfpr="u" 'LFPR type -- [a]djusted (age-sex adjusted) or [u]nadjusted.
%lfpr="a"

'*****END of section that needs updating for each run*****


!startyr=!TRyr-17 'defines the start date for the workfile; need it to be far enough in the past to catch all lagged values

'define the sample period for which LFPR decomposition will  be done
%datestart=@str(!TRyr-1)+"q4" ' Q4 of year (TRyr-1) 
...
%tr_pr=@str(!TRyr-2000-1) 'string variable with previous TR year, i.e. "16" when the current TR is 2017; need this for previous year's O-bank.
...
%dbank="dtr"+%trstr+%alt
%opbank="op1"+%tr_pr+%alt+"o" 'Note that this is O-bank from previous-year TR, i.e. op1162o.bnk when the current TR is 2017
%addbank="adtr"+%trstr+%alt

%abankpath=%folder+"secret place\"+%abank+".bnk"  'full path to the databank file; includes file extension 
%dbankpath=%folder+"secret place\"+%dbank+".bnk" 
%opbankpath=%folder+"MKS\"+%opbank+".bnk" 
%addbankpath=%folder+"MKS\"+%addbank+".bnk" 


wfcreate(wf=lfpr_decomp, page=Qdata) q !startyr !endyr 'create workfile with quarterly data between !startyr Q1 and !endyr Q4. 
...
copy lfpr_coefs::\lfprbetas lfpr_decomp::Qdata\  'lfprbetas is the matrix of estimated coefficicnts
copy lfpr_coefs::\lfprgroups lfpr_decomp::Qdata\  'lfprgroups is the string vector of group names, where the order of elements indicates the corresponding row in lfprbetas
wfclose %LFPRestimates
...
'all aggregated groups in a specific order
...
fetch(d={%addbank}) lc.adj pf65.add pf67.add pm67.add pm72.add pm74.add

close @db

series tot_lf_adj=lc_adj/(lc-lc_adj) 'total labor force adjustment factor; it will be used much later in the program. It is common to all groups, so needs to be defined only once. 

'for Adjusted LFPRs, reset certain series to a constant value equat to TRyr-1 Q4; for unadjusted LFPRs this step is skipped
if %lfpr="a" then 
...
if %grg="f" then
	...
	%list_op="rf*di.q"   'series to be fetched from O-bank(previous TRyr)
	else
...
%list_add="pf65.add pf67.add pm67.add pm72.add pm74.add" 'this is a complete set of addfactors we have, so no need to differentiate by gender 
...
'for Adjusted LFPRs, reset marital status and child presence series to a constant value equat to TRyr-1 Q4; for unadjusted LFPRs this step is skipped
if %lfpr="a" then 
...
'compute ga (global addfactor, i.e. total labor force adjustment), tb (total base)
series {%groupfull}_ga=tot_lf_adj*({%groupfull}_in+{%groupfull}_le+{%groupfull}_ia+{%groupfull}_ag+{%groupfull}_ge+{%groupfull}_ms+{%groupfull}_ch+{%groupfull}_bc+{%groupfull}_di+{%groupfull}_ed+{%groupfull}_rr+{%groupfull}_et+{%groupfull}_pf+{%groupfull}_coh+{%groupfull}_lc+{%groupfull}_tr) 'total labor force effect
...
next '**********end of the main loop through all PRIMARY demo groups********************
...
'for Adjusted LFPRs, reset population to a constant value equat to TRyr-1 Q4; for unadjusted LFPRs this step is skipped
if %lfpr="a" then 
	for %ser {%pop_list}
	{%ser} = @elem({%ser}, %datestart)
	next
...
spool report 'declare spool to display messages
...
'format the table 
if %lfpr="a" then 
	%title = "LFPR Decomposition "+%g+" TR"+@str(!TRyr)+" alt"+%alt+" (adjusted for age, sex, marital status, presence of children)"
...
lfpr_{%g}.setlines(A8:U91) +o
lfpr_{%g}.setlines(A8:A91) +r
lfpr_{%g}.setlines(C8:C91) +o
lfpr_{%g}.setlines(G8:G91) +o
lfpr_{%g}.setlines(U8:U91) +l
...
report.display 'disply on screen the spool that would indicate if we found any non-zero residuals

pageselect results

'wfsave LFPR_decomp 'saves the workfile 
